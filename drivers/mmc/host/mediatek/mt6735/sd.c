...
#include <mach/items.h>  //elink zhou 20160420 add 
...
int msdc_of_parse(struct mmc_host *mmc)
{
	...
	* property data isn't large enough.*/
	...
	//Elink zhou 20160420 add  add begin for sdcard hotplug level 
	if(host->id == 1){ //msdc1
		if(item_integer("sdcard.hotplug.level",0) == 1)
			host->hw->cd_level = MSDC_CD_HIGH;
		else
			host->hw->cd_level = MSDC_CD_LOW;
		pr_err("[Elink Nicholas Zheng][MSDC]items kernel: msdc get irq level = %d\n", host->hw->cd_level);
	}	
	//END
	/*get cd_gpio*/
	of_property_read_u32_index(np, "cd-gpios", 1, &cd_gpio);

	//Elink zhou 20160420  add begin for sdcard hotplug gpio 
	if(host->id == 1){ //msdc1
		cd_gpio = item_integer("sdcard.hotplug.gpio",0);	
		pr_err("[Elink Nicholas Zheng][MSDC]items kernel: msdc get irq gpio = %d\n", cd_gpio);
	}	
	//END	
