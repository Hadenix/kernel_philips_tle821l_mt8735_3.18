...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	params->dsi.vertical_backporch					= 23; //23;
	params->dsi.vertical_frontporch 				= 12; //12;
	params->dsi.vertical_active_line				= FRAME_HEIGHT; 

	params->dsi.horizontal_sync_active				= 10;
	params->dsi.horizontal_backporch				= 160; //160;
	params->dsi.horizontal_frontporch				= 160; //160;
	params->dsi.horizontal_active_pixel 			= FRAME_WIDTH;

	params->dsi.PLL_CLOCK=180; //145;//160; 
...
//#if 1
//if(	item_equal("LCM.type", "CPT", 0) || item_equal("LCM.type", "IVO", 0)){
	// CPT / IVO 
	...
	MDELAY(1);
//}
//else if( item_equal("LCM.type", "BOE", 0) ){
//#endif	
	// BOE
/*	data_array[0] = 0x8B801500;
	...
	dsi_set_cmdq(&data_array, 1, 1);
	MDELAY(1);
}else if( item_equal("LCM.type", "BOE_W109", 0) ){
	data_array[0] = 0xbb801500;
	dsi_set_cmdq(&data_array, 1, 1);
	MDELAY(1);

	data_array[0] = 0xbb811500;
	dsi_set_cmdq(&data_array, 1, 1);
	MDELAY(1);

	data_array[0] = 0xbb821500;
	dsi_set_cmdq(&data_array, 1, 1);
	MDELAY(1);
...
void tf070mc124_ips_early_init_lcm(void)
{
