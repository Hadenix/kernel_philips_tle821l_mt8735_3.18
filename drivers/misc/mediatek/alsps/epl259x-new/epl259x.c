...
static int alsps_local_init(void)
{
	...
	//printk("[ALS/PS] alsp.compatible = %s \n" , epl_sensor_i2c_driver.driver.name);
	//strcpy(temp, epl_sensor_i2c_driver.driver.of_match_table.compatible);
	//printk("[ALS/PS] alsp.compatible = %s \n" , temp);
	//printk("[ALS/PS] alsp.compatible = %s \n" , epl_sensor_i2c_driver.driver.of_match_table.type);
	//printk("[ALS/PS] alsp.compatible = %s \n" , epl_sensor_i2c_driver.driver.of_match_table.compatible);
	//printk("[ALS/PS] alsp.compatible = %d \n" , epl_sensor_i2c_driver.driver.of_match_table.data);	
...
static int __init epl_sensor_init(void)
{
    ...
    if(!item_equal("alsps.ic.model", "epl259x", 0)){
		APS_ERR("[Nicholas Zheng] ALSPS IC model is not epl259x \n");	
		return 0;
    }
