...
//	#define STK_MAX_MIN_DIFF	(650)
//	#define STK_LT_N_CT	(500)
//	#define STK_HT_N_CT	 (600)
...
/*----------------------------------------------------------------------------*/
struct i2c_client *stk3x1x_i2c_client = NULL;
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
	...
	{
		if(als_data < STK_ALS_THRESHOLD && obj->als_code_last > 5000)	//10000
		{
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_PS_DATA:    
			...
			APS_LOG("%s:ps raw 0x%x -> 11 value 0x%x \n",__FUNCTION__, obj->ps, dat);			
#endif			
...
//	sensor_data = (hwm_sensor_data *)buff_out;				
	...
	else
	{
		...
		if(data < 0)
		{
			APS_LOG("stk_ps_get_data read error\n");
			err = -1;
		}
		else
		{
...
//			sensor_data->value_divide = 1;
			*status = SENSOR_STATUS_ACCURACY_MEDIUM;
#ifdef STK_PS_POLLING_LOG						
			APS_LOG("%s:11 ps raw 0x%x -> value 0x%x \n",__FUNCTION__, obj->ps, data);					
#endif				
		}	
	}	
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw->i2c_addr[0] = 0x90;
	client->addr = 0x90 >> 1;
	//Elink Nicholas Zheng add end
...
static int __init stk3x1x_init(void)
{
	...
	struct i2c_board_info i2c_stk3x1x={ I2C_BOARD_INFO("stk3x1x",hw->i2c_addr[0] )};

	if(item_equal("alsps.ic.model", "epl259x", 0)){
		APS_ERR("[Nicholas Zheng] ALSPS IC model is not stk3x1x \n");
		return 0;
	}
	   
	APS_FUN();
	hw = get_alsps_dts_func(name, hw);
	hw = get_alsps_items_func(name, hw);
	
	if (!hw) {
		APS_ERR("get_alsps_dts_func fail\n");
		return 0;
	}
	printk("lanhai %s: i2c_number=%d, i2c_addr: 0x%x\n", __func__, hw->i2c_num, hw->i2c_addr[0]);
