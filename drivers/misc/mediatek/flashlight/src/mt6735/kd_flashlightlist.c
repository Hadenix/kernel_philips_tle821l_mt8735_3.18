...
static struct pinctrl_state *flashlight_mode_high;
static struct pinctrl_state *flashlight_mode_low;
...
int flashlight_gpio_init(struct platform_device *pdev)
{
	...
	//mode pin
	flashlight_mode_high = pinctrl_lookup_state(flashlight_pinctrl, "mode_high");
	if (IS_ERR(flashlight_mode_high)) {
		ret = PTR_ERR(flashlight_mode_high);
		logI("%s : pinctrl err, flashlight_mode_high\n", __func__);
	}

	flashlight_mode_low = pinctrl_lookup_state(flashlight_pinctrl, "mode_low");
	if (IS_ERR(flashlight_mode_low)) {
		ret = PTR_ERR(flashlight_mode_low);
		logI("%s : pinctrl err, flashlight_mode_low\n", __func__);
	}
	return ret;
}
...
{
	return pinctrl_select_state(flashlight_pinctrl, flashlight_mode_high);
}

int flashlight_gpio_mode_low(void)
{
	return pinctrl_select_state(flashlight_pinctrl, flashlight_mode_low);
}

EXPORT_SYMBOL(flashlight_gpio_enable_high);
EXPORT_SYMBOL(flashlight_gpio_enable_low);
EXPORT_SYMBOL(flashlight_gpio_mode_high);
EXPORT_SYMBOL(flashlight_gpio_mode_low);
/* ============================== */
/* functions */
/* ============================== */
int globalInit(void)
