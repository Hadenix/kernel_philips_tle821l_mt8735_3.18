...
static struct i2c_board_info __initdata i2c_ncp1854 = { I2C_BOARD_INFO("ncp1854", (NCP1854_SLAVE_ADDR_WRITE>>1))};
...
static void ncp1854_parse_customer_setting(void)
{
	...
	if (np) {
		if (of_property_read_u32(np, "disable_ncp1854_fctry_mod", &val) == 0) {
			...
			battery_log(BAT_LOG_CRTI, "%s: disable factory mode, %d\n", __func__, val);
		}
	}
	...
	pinctrl_drvvbus_init = pinctrl_lookup_state(pinctrl, "drvvbus_low");
...
static int ncp1854_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ncp1854_set_fctry_mode(0x0); // Elink Nicholas Zheng add 
	
	ncp1854_dump_register(); // Elink Nicholas Zheng add 
	
	battery_log(BAT_LOG_CRTI, "[ncp1854_driver_probe done]\n");
	return 0;

exit:
	battery_log(BAT_LOG_CRTI, "[ncp1854_driver_probe error]\n");
	return err;
...
static int __init ncp1854_init(void)
{
    ...
    ret = i2c_register_board_info(NCP1854_BUSNUM, &i2c_ncp1854, 1);
	if (ret)
		printk("[ncp1854_init] failed to i2c_register_board_info to NCP1854\n");
	else
		printk("[ncp1854_init] Success to i2c_register_board_info to NCP1854\n");
