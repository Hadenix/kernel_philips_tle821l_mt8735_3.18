...
#define CONFIG_Z703_LCM_IS_JD9366
#ifdef CONFIG_Z703_LCM_IS_JD9366
unsigned int g_lcm_id=0x9366;
unsigned int  is_JD9366_need_recover =0;
#endif
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	DSI_T0_INS t0;
	DSI_T0_INS t1;//lhh added
	struct DSI_RX_DATA_REG read_data0;
	struct DSI_RX_DATA_REG read_data1;//lhh added
        struct DSI_RX_DATA_REG read_data2;
        struct DSI_RX_DATA_REG read_data3;
	uint32_t recv_data_cnt = 0; //lhh added
        unsigned char packet_type; //lhh added
	static cmdqBackupSlotHandle hSlot[4] ={0, 0, 0, 0};//lhh modis
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
			...
			//DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[0], 0x01e01500);
			//DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[1], 0x354a1500);
			#ifdef CONFIG_Z703_LCM_IS_JD9366
			if(0x9366==g_lcm_id)
				{
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[0], 0x02E01500);
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[1], 0x41081500);
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[2], 0x430f1500);//0x430f1500
				//   	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[3], 0x4a1b1500);//0x4a1b1500
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[4], 0x1f281500);//0x1f281500
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[5], 0x62631500);//0x62631500
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[6], 0x62641500);//0x62641500
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[7], 0x44301500);//0x62641500
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[8], 0x46321500);//0x62641500
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[9], 0x42341500);//0x62641500
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[10], 0x403b1500);//0x62641500
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[11], 0x45461500);//0x62641500
				//	DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[12], 0x00E01500);//0x62641500
					...
					printk("%s ,lcm_id==%x,LINE ==%d\n",__FUNCTION__,g_lcm_id,__LINE__);
				}
				else
					{
						DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[0], AS_UINT32(&t1));
			...
			//	DSI_BACKUPREG32(cmdq_trigger_handle, hSlot[i], i,&DSI_REG[0]->DSI_RX_DATA0);
			}
		...
		for (i = 0; i < 3; i++) {
			...
			if (NULL!=hSlot) {
				...
				cmdqBackupReadSlot(hSlot[1], i, ((uint32_t *) &read_data1));//lhh added
				cmdqBackupReadSlot(hSlot[2], i, ((uint32_t *) &read_data2));//lhh added
				cmdqBackupReadSlot(hSlot[3], i, ((uint32_t *) &read_data3));//lhh added
			} else {
				/* read from dsi , support only one cmd read */
				if (i == 0) {
					DSI_OUTREG32(NULL, &read_data0,AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA0));
  					DSI_OUTREG32(NULL, &read_data1, AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA1));//lhh added
                    			DSI_OUTREG32(NULL, &read_data2, AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA2));//lhh added
                    			DSI_OUTREG32(NULL, &read_data3, AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA3));//lhh added
				}
			}
			...
			DISPCHECK("[DSI]cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x para_list[2]=0x%x\n",
				...
				AS_UINT32(DDP_REG_BASE_DSI0 + 0x0c)); */
     				packet_type = read_data0.byte0;
                        ...
                        if(packet_type == 0x1A || packet_type == 0x1C)
                        {
                                ...
                                if(recv_data_cnt==3)
				 {
						DISPCHECK("lhh  line ==%d\n",__LINE__);
					    if((read_data1.byte0== dsi_params->lcm_esd_check_table[i].para_list[0])&&(read_data1.byte1 == dsi_params->lcm_esd_check_table[i].para_list[1])&&(read_data1.byte2 == dsi_params->lcm_esd_check_table[i].para_list[2]))
				/* clear rx data */
				/* DSI_OUTREG32(NULL, &DSI_REG[dsi_i]->DSI_RX_DATA0,0); */
        				{	
        					ret = 0; // esd pass
        					 DISPCHECK("[DSI]read_data1.byte0 = 0x%x, ret = %d ,dsi_params->lcm_esd_check_table[i].para_list[0] = 0x%x\n",read_data1.byte0,ret,dsi_params->lcm_esd_check_table[i].para_list[0]);
                                        	 DISPCHECK("[DSI]lhh  line ==%d ,recv_data_cnt = %d\n",__LINE__,recv_data_cnt);
					      	 break;
					    	}else{
								ret = 1; // esd fail
						                DISPCHECK("lhh  line ==%d\n",__LINE__);
				 ...
				 if(recv_data_cnt==1)
					{
						if(read_data1.byte0== dsi_params->lcm_esd_check_table[i].para_list[0])
						{
								...
								ret = 1; // esd fail
						                DISPCHECK("[DSI]lhh  line ==%d ,recv_data_cnt = %d\n",__LINE__,recv_data_cnt);
								#ifdef CONFIG_Z703_LCM_IS_JD9366
					 			if(0x0a ==dsi_params->lcm_esd_check_table[i].cmd)
								 	{
	 							               printk("lhh  line ==%d\n",__LINE__);
								 		is_JD9366_need_recover =1;
								 	}
								#endif
        							break;
						}
        			...
        			{
        			  DISPCHECK("[DSI]read_data0.byte1 = 0x%x, ret = %d ,dsi_params->lcm_esd_check_table[i].para_list[0] = 0x%x\n",read_data0.byte1,ret,dsi_params->lcm_esd_check_table[i].para_list[0]);
    				  DISPCHECK("[DSI]lhh  line ==%d ,recv_data_cnt = %d\n",__LINE__,recv_data_cnt);
        				// clear rx data
        				// DSI_OUTREG32(NULL, &DSI_REG[dsi_i]->DSI_RX_DATA0,0);
        				ret = 0; // esd pass
...
//		cmdqBackupAllocateSlot(&hSlot, 3);
                ...
                cmdqBackupAllocateSlot(&hSlot[1], 3);//lhh added
...
//	    	hSlot = 0;
                        ...
                        cmdqBackupFreeSlot(hSlot[1]);//lhh added
