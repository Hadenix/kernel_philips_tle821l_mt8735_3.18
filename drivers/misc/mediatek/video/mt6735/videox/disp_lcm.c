...
bool lcd_hall_status = false; //Elink zhou  add 
...
int disp_lcm_suspend(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
		if (lcm_drv->suspend) {
			lcd_hall_status = false; // Elink zhou add
			printk("hall [%s]set lcd_hall_status <%d>\n", __func__, lcd_hall_status);
			lcm_drv->suspend();
		} else {
...
int disp_lcm_resume(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->resume) {
			lcd_hall_status = true; // Elink zhou add
			printk("hall [%s]set lcd_hall_status <%d>\n", __func__, lcd_hall_status);
			lcm_drv->resume();
		} else {
