...
#ifdef EK_YUV_CAM_DEBUG
#define DEBUG_SENSOR_GC0308
#endif
...
gc0308_initial_set_struct gc0308_Init_Reg[5000];
static u32 strtol(const char *nptr, u8 base)
{
	printk("gc0308___%s____\n",__func__); 
	u8 ret;
	if(!nptr || (base!=16 && base!=10 && base!=8))
	{
		printk("gc0308 %s(): NULL pointer input\n", __FUNCTION__);
		return -1;
	}
	for(ret=0; *nptr; nptr++)
	{
...
static u8 GC0308_Initialize_from_T_Flash()
{
	u8 *curr_ptr = NULL;
	u32 file_size = 0;
	u32 i = 0, j = 0;
	u8 func_ind[4] = {0};	/* REG or DLY */
	printk("gc0308___%s____11111111111111\n",__func__); 
	struct file *fp; 
	mm_segment_t fs; 
	loff_t pos = 0; 
	static u8 data_buff[10*1024] ;
	fp = filp_open("/mnt/sdcard/gc0308_sd.txt", O_RDONLY , 0); 
	if (IS_ERR(fp)) 
	{ 
		printk("0308 create file error 1111111\n");  
		return -1; 
	} 
	else
	{
		printk("0308 create file error 2222222\n");  
	}
	fs = get_fs(); 
	set_fs(KERNEL_DS); 
	...
	filp_close(fp, NULL); 
	set_fs(fs);
	printk("gc0308___%s____22222222222222222\n",__func__); 
	curr_ptr = data_buff;
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			gc0308_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */
			gc0308_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		}
		else									/* DLY */
		{
			...
			gc0308_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
		while (!((*curr_ptr == 0x0D) && (*(curr_ptr+1) == 0x0A)))
		{
	...
	i++;
	printk("gc0308 %x  ==  %x\n",gc0308_Init_Reg[j].init_reg, gc0308_Init_Reg[j].init_val);
	printk("gc0308___%s____3333333333333333\n",__func__); 
#if 1
	for (j=0; j<i; j++)
	{
		if (gc0308_Init_Reg[j].op_code == gc0308_OP_CODE_END)	/* End of the setting. */
		{
			printk("gc0308 REG OK -----------------END!\n");
			break ;
		}
		else if (gc0308_Init_Reg[j].op_code == gc0308_OP_CODE_DLY)
		{
			msleep(gc0308_Init_Reg[j].init_val);		/* Delay */
			printk("gc0308 REG OK -----------------DLY!\n");			
		}
		else if (gc0308_Init_Reg[j].op_code == gc0308_OP_CODE_REG)
		{
			...
			printk("gc0308 REG OK!-----------------REG(0x%x,0x%x)\n",gc0308_Init_Reg[j].init_reg, gc0308_Init_Reg[j].init_val);			
			printk("gc0308 REG OK!-----------------REG(0x%x,0x%x)\n",gc0308_Init_Reg[j].init_reg, gc0308_Init_Reg[j].init_val);			
			printk("gc0308 REG OK!-----------------REG(0x%x,0x%x)\n",gc0308_Init_Reg[j].init_reg, gc0308_Init_Reg[j].init_val);			
		}
		else
		{
			printk("gc0308 REG ERROR!\n");
		}
	}
#endif
	return 1;	
...
UINT32 GC0308Open(void)
{
    ...
    GC0308_Write_More_Registers();
		...
		if(fromsd == 1)
		{
			printk("________________0308 from t!\n");
			printk("________________0308 from t!\n");
			printk("________________0308 from t!\n");		
			GC0308_Initialize_from_T_Flash();
			printk("______after_____0308 from t!\n");	
		}
		else
		{
			printk("________________0308 not from t!\n");	
			printk("________________0308 not from t!\n");
			printk("________________0308 not from t!\n");		
			RETAILMSG(1, (TEXT("Sensor Read ID OK \r\n"))); 
		}
...
{
#ifdef DEBUG_SENSOR_GC0308
		printk("______%s______GC0308 YUV setting\n",__func__);
		return TRUE;
#endif
    switch (iCmd) {
    case FID_AWB_MODE:
...
//	unsigned long long *feature_return_para=(unsigned long long *) pFeaturePara;
    ...
    case SENSOR_FEATURE_SET_YUV_CMD:
        GC0308YUVSensorSetting((FEATURE_ID)*feature_data, *(feature_data+1));
        break;
    case SENSOR_FEATURE_SET_VIDEO_MODE:    //  lanking
	 GC0308YUVSetVideoMode(*feature_data);
	 break;
