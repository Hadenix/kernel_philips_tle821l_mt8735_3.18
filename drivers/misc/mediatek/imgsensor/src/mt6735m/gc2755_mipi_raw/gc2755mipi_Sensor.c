...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2755MIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define ANALOG_GAIN_7 428  // 6.68x
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
			...
			printk("GC2755 analogic gain 1x , GC2755 add pregain = %d\n",temp);
	...
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
			...
			write_cmos_sensor(0xb1, temp>>6);
			write_cmos_sensor(0xb2, (temp<<2)&0xfc);
			printk("GC2755 analogic gain 1.45x , GC2755 add pregain = %d\n",temp);
	...
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
			...
			write_cmos_sensor(0xb1, temp>>6);
			write_cmos_sensor(0xb2, (temp<<2)&0xfc);
			printk("GC2755 analogic gain 2.02x , GC2755 add pregain = %d\n",temp); 
	}
	...
	else
	{
			...
			printk("GC2755 analogic gain 2.8x, temp = %d\n", temp);
	}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, 0x15);//前摄
	...
	write_cmos_sensor(0x94, 0x06);//08
