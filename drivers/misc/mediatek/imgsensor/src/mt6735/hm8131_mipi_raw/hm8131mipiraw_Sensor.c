...
static void HM8131MIPI_Set_Dummy(const kal_uint32 iPixels, const kal_uint32 iLines)
{
	...
	//printk( ">>> HM8131MIPI_Set_Dummy(): iPixels=%d,iLines=%d\n",iPixels ,iLines);
	...
	if(LineLength !=HM8131SensorDriver.LineLength){
		if(LineLength > HM8131MIPI_MAX_LINE_LENGTH){
			printk("Set Dummy Error\n ");
			return;
		}
		HM8131SensorDriver.LineLength = LineLength;
		printk( "HM8131MIPI_Set_Dummy(): LineLength Change to =%d\n", LineLength );
	...
	if(FrameHeight !=HM8131SensorDriver.FrameHeight){
		...
		printk( "HM8131MIPI_Set_Dummy(): FrameHeight Change to =%d\n", FrameHeight );
...
{	
	...
	printk("[ABC]iShutter=%d,HM8131SensorDriver.FrameHeight=%d",iShutter,HM8131SensorDriver.FrameHeight);
...
{
	printk( ">>> HM8131MIPI_SetShutter(): iShutter=0x%X, %d\n", iShutter, iShutter );
...
void HM8131MIPI_SetGain(UINT16 iGain)
{
	...
	printk( ">>> HM8131MIPI_SetGain(): iGain=0x%X\n", iGain );
...
kal_uint16 read_HM8131_gain(void)
{
	...
	printk( "HM8131SensorDriver.Gain2Reg=0x%X, HM8131SensorDriver.Reg2Gain=%d\n", HM8131SensorDriver.Gain2Reg, HM8131SensorDriver.Reg2Gain );
...
SUC:	
	...
	printk( ">> Apply HM8131InitSetting\n" );
...
SUC:	
	if( HM8131SensorDriver.SensorID != HM8131MIPI_SENSOR_ID ) {
		printk("GetSensorID Erro!!!\n");
		*sensorID = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}else{
		printk("GetSensorID Success!!!\n");	
...
//	kal_uint16 dummy_line = 0;
	...
	if(HM8131SensorDriver.SensorMode != SENSOR_MODE_PREVIEW){
#if 0	
		printk( ">> Apply HM8131_1MSettings\n" );
		...
		printk( "<< Apply HM8131_1MSettings\n" );
#endif
		printk( ">> Apply HM8131_1080PSettings\n" );
		HM8131MIPI_write_cmos_sensor(0x0100,0x00);
...
UINT32 HM8131MIPISetVideoMode(UINT16 u2FrameRate)
{
	...
	printk( ">>> HM8131MIPISetVideoMode(): frame rate=%d\n", u2FrameRate );
	...
	if( u2FrameRate > 30 || u2FrameRate < 5 )
		printk( "Error frame rate setting\n" );
	...
	printk( "HM8131MIPISetVideoMode(): FrameLength Change to =%d\n", FrameLength );
...
UINT32 HM8131SetAutoFlickerMode(kal_bool bEnable, UINT16 u2FrameRate)
{
	...
	printk( ">>> HM8131SetAutoFlickerMode(): Frame rate(10base)= %d, %d\n", bEnable, u2FrameRate );
	...
	printk( "<<< HM8131SetAutoFlickerMode()\n" );
...
{
	printk("[HM8131SetTestPatternMode] Test pattern enable:%d\n", bEnable);
...
UINT32 HM8131MIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) 
{
	...
	printk( ">>> SetMaxFramerateByScenario(): scenarioId=%d, frame rate=%d\n", scenarioId, frameRate );
	...
	printk( "<<< SetMaxFramerateByScenario()\n");
...
{
	printk( ">>> GetDefaultFramerateByScenario()\n");
	...
	*pframeRate = HM8131SensorDriver.SensorFps;
	...
	printk( "<<< GetDefaultFramerateByScenario(): FrameRate=%d\n",*pframeRate);
	return ERROR_NONE;
}
...
//derek
  ...
  //  unsigned long long *feature_return_para=(unsigned long long *) feature_para;
	...
	printk(">>> FeatureControl():FeatureId=%x\n",FeatureId);
	switch( FeatureId )
	{
...
UINT32 HM8131_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
