...
int batt_meter_init_cust_data(void)
{
	...
	__elink_meter_init_cust_data_from_items(); //Elink_yg@20160308

	//for test
	printk(" ELink_yg [%s] batt_meter_cust_data.q_max_pos_25 = %d\n", __func__, batt_meter_cust_data.q_max_pos_25);
...
#else
	return sizeof(elink_battery_profile_temperature) / sizeof(BATTERY_PROFILE_STRUCT); //ELink_yg@20160307 add
#endif
}
...
#else
	return sizeof(elink_r_profile_temperature) / sizeof(BATTERY_PROFILE_STRUCT); //ELink_yg@20160307 add
#endif
}
...
BATTERY_PROFILE_STRUCT_P fgauge_get_profile(unsigned int temperature)
{
	...
	if (temperature == batt_meter_cust_data.temperature_t)
		return &battery_profile_temperature[0];
#else
	if (temperature == batt_meter_cust_data.temperature_t2) //ELink_yg@20160307 add
		return __elink_fgauge_get_profile_in_items();//&elink_battery_profile_t2[0];
	else
		return &elink_battery_profile_temperature[0];
#endif
	return NULL;
...
#else
	if (temperature == batt_meter_cust_data.temperature_t2) //ELink_yg@20160307 add
		return __elink_fgauge_get_profile_r_table_in_items();//&elink_r_profile_t2[0];
	else
		return &elink_r_profile_temperature[0];
#endif
	return NULL;
}
#endif
...
void fgauge_construct_battery_profile(signed int temperature, BATTERY_PROFILE_STRUCT_P temp_profile_p)
{
	...
	elink_fgauge_construct_battery_profile(temp_profile_p); //ELink_yg@20160307

	/* Dumpt new battery profile*/
	for (i = 0; i < saddles; i++) {
		printk("[%s] <DOD,Voltage> at %d = <%d,%d>\r\n", __func__,
...
void fgauge_construct_r_table_profile(signed int temperature, R_PROFILE_STRUCT_P temp_profile_p)
{
	...
	elink_fgauge_construct_r_table_profile(temp_profile_p); //ELink_yg@20160307

	/* Dumpt new r-table profile */
#if 1//defined(BATTERY_DEBUG) //ELink_yg@20160307 modify
	for (i = 0; i < saddles; i++) {
		printk("[%s] <Rbat,VBAT> at %d = <%d,%d>\r\n",__func__,
...
#if !defined(USE_NTC_BATTERY)
	elink_get_batt_meter_cust_data(&batt_meter_cust_data); //ELink_yg@20160308

	Elink_items_config_BAT(); //ELink_yg@20160308
#endif
