...
/****** Elink Nicholas Modif for Analog gain begin******/
static void HeadsetVoloumeSetElink(void)
{
	int index = 0, oldindex = 0, offset = 0, count = 1;
	...
	oldindex = 8;
	if (index > oldindex) {
		pr_warn("[HeadsetVoloumeSetElink]%s index = %d oldindex = %d\n", __func__ , index, oldindex);
		offset = index - oldindex;
		while (offset > 0) {
			Ana_Set_Reg(ZCD_CON2, ((oldindex + count) << 7) | (oldindex + count),
				    0xf9f);
...
/****** Elink Nicholas Modif for Analog gain END******/
static void Audio_Amp_Change(int channels, bool enable)
{
	if (enable) {
...
#endif /*CONFIG_MTK_LEGACY*/		
#endif
		AudDrv_GPIO_EXTAMP_Select(true); // elink nicholas zheng add 
...
#endif		
#endif
		AudDrv_GPIO_EXTAMP_Select(false); // elink nicholas zheng add 
		udelay(500);
		pr_debug("Ext_Speaker_Amp_Change OFF-\n");
	}
#endif
...
static void Headset_Speaker_Amp_Change(bool enable)
{
	if (enable) {
		...
		/* apply volume setting */
		//HeadsetVoloumeSet();
		HeadsetVoloumeSetElink(); //Elink_Matt@20161101 add
		Apply_Speaker_Gain();
	} else {
#ifdef CONFIG_MTK_SPEAKER
		if (Speaker_mode == AUDIO_SPEAKER_MODE_D)
